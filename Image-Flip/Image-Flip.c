...
int main(int argc, char const *argv[])
{
    ...
    // output data
    ImgRGB **output1_flip = RGB_2D(H1, W1);
    ImgRGBA **output2_flip = RGBA_2D(H2, W2);
    ...
    for(int i=0; i<H1; i++){
        for(int j = 0; j<W1; j++){
            output1_flip[i][j].R = input1_RGB[i][W1-j].R;
            output1_flip[i][j].G = input1_RGB[i][W1-j].G;
            output1_flip[i][j].B = input1_RGB[i][W1-j].B;
        }
    }

    for(int i=0; i<H2; i++){
        for(int j = 0; j<W2; j++){
            output2_flip[i][j].R = input2_RGBA[i][W2-j].R;
            output2_flip[i][j].G = input2_RGBA[i][W2-j].G;
            output2_flip[i][j].B = input2_RGBA[i][W2-j].B;
            output2_flip[i][j].A = input2_RGBA[i][W2-j].A;
        ;
        }
    }
    writebmpRGB("output1_flip.bmp", &bmpheader1, output1_flip, skip);
    writebmpRGBA("output2_flip.bmp", &bmpheader2, output2_flip);
